...
pub struct Entry {
    ...
    /// UID of the entry.
    _identifier: std::vec::Vec<u8>, // 16 bytes

    /// Flags of the entry.
    _flags: std::vec::Vec<u8>, // 4 bytes
...
impl Entry {
    fn from_slice(sector: &[u8], dir_id: u32) -> Result<Entry, Error> {
        ...
        let entry = Entry {
            ...
            _identifier: sector[80..96].to_vec(),
            _flags: sector[96..100].to_vec(),
...
impl<'ole> crate::ole::Reader<'ole> {
        ...
        if size == 0 {
            ...
            Ok(
                if &size < self.minimum_standard_stream_size.as_ref().unwrap() {
                    self.get_short_stream_slices(&entry.sec_id_chain, size)?
                } else {
                    self.get_stream_slices(&entry.sec_id_chain, size)?
                },
            )
        }
    }
    ...
    pub(crate) fn build_directory_entries(&mut self) -> Result<(), Error> {
        let n_entry_by_sector = self.sec_size.as_ref().unwrap() / constants::DIRECTORY_ENTRY_SIZE;
        let mut entries =
            Vec::<Entry>::with_capacity(self.dsat.as_ref().unwrap().len() * n_entry_by_sector);
