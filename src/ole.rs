...
impl<'ole> Reader<'ole> {
    ...
    /// ```
    pub fn new<T: 'ole>(readable: T) -> Result<Reader<'ole>, crate::error::Error>
    where
        T: std::io::Read,
    {
        let mut t = Reader {
            buf_reader: Some(std::io::BufReader::new(Box::new(readable))),
            uid: vec![0u8; crate::constants::UID_SIZE],
    ...
    /// }
    /// ```
    pub fn iterate(&self) -> crate::iterator::OLEIterator {
        crate::iterator::OLEIterator::new(self)
    }
...
mod tests {
    ...
    fn array_bad_identifier() {
        let mut vec = crate::constants::IDENTIFIER.to_vec();
        vec[0] = 0xD1;
        fill(&mut vec);
    ...
    fn fill(buf: &mut std::vec::Vec<u8>) {
        let missing = vec![0u8; crate::constants::HEADER_SIZE - buf.len()];
        buf.extend(missing);
    }
    ...
    fn bad_sec_size() {
        ...
        vec.extend(&crate::constants::LITTLE_ENDIAN_IDENTIFIER);
